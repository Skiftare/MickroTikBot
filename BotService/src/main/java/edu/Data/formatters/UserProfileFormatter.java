...
public final class UserProfileFormatter {
    ...
    private String formatClientInfo(final UserInfo userInfo) {
        ...
        if (client.balance() != null) {
            result.append("💰 Баланс: ").append(client.balance().toPlainString()).append(ENDL_STRING);
        }

        if (client.vpnProfile() != null) {
            try {
                result.append(ENDL_STRING);
                result.append("🔐 VPN профиль: ").append(formatCredentialsForConnection(EncryptionUtil.decrypt(
                        client.vpnProfile()))
                );
            } catch (Exception e) {
    ...
    public static String formatCredentialsForConnection(String incomedString) {
        ...
        for (String line : lines) {
            ...
            if (line.startsWith(LOGIN_STRING)) {
                bufferedLine = formatHtmlLine(line, LOGIN_STRING);
            } else if (line.startsWith(PASSWORD_STRING)) {
                bufferedLine = formatHtmlLine(line, PASSWORD_STRING);
            } else if (line.startsWith(SECRET_STRING)) {
                bufferedLine = formatHtmlLine(line, SECRET_STRING);
            }
    ...
    private static String formatHtmlLine(String line, String prefix) {
        String value = line.substring(prefix.length()).trim();
        String htmlFormatter = "<code>";  // HTML-тег для форматирования
        return prefix + " " + htmlFormatter + value + "</code>";
